...
export default function App() {
  ...
  const [metaLearning, setMetaLearning] = useState([])
  ...
  const refreshEvolutionHistory = async () => {
    try {
      ...
      setMetaLearning(await getMetaLearning())
    } catch {
      setEvolutionHistory([])
      setMetaLearning([])
    }
  ...
  const handleEvolveWinners = async () => {
    setLoading(true)
    try {
      await evolveWinners({ version_group: versionGroup, top_k: 2, next_version_group: nextVersionGroup })
      await refreshEvolutionHistory()
  ...
  const state = {
    file, text, ingestedText, versionGroup, nextVersionGroup, flows, allFlows,
    executions, evaluations, winners, selectedFlowId, evolutionHistory, performancePoints,
    metaLearning, loading,
  }

  const actions = {
    setFile, setText, setVersionGroup, setNextVersionGroup, setSelectedFlowId,
    handleIngest, handleGenerateVariants, handleRunCompetition, handleSelectWinners, handleEvolveWinners,
  }
